...
export default async function ModulosPage() {
  ...
  const modules: Array<{
    ...
    {
      title: 'Comissoes',
      description: 'Gestao de comissoes, lancamentos, relatorios e campanhas',
      href: '/dashboard',
      icon: 'chart',
    },
  ]
  ...
  if (hasWaAccess) {
    modules.push({
      ...
      description: 'Inbox de conversas, bots e automacoes de atendimento',
      href: '/wa',
      icon: 'chat',
    })
  }
        ...
        {modules.map((mod) => (
          ...
          >
            ...
            <div className="w-12 h-12 rounded-lg bg-verde-claro flex items-center justify-center mb-4 group-hover:bg-verde transition-colors">
              {mod.icon === 'chart' ? (
                <svg
                  className="w-6 h-6 text-verde group-hover:text-white transition-colors"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
